...
def generate_ab_test_cohorts(input_csv="data/processed/churn_predictions_v2.csv"):
    if not os.path.exists(input_csv):
        input_csv = (
            ...
            else "churn_predictions_v2.csv"
        )
        if not os.path.exists(input_csv):
            raise FileNotFoundError(
                "Neither 'data/processed/churn_predictions_v2.csv' nor 'churn_predictions.csv' was found."
            )
    ...
    print(
        f"[INFO] Identified {total_target_customers} high-risk Month-to-Month subscribers (P > 0.50)."
    )

    if total_target_customers < 2:
        print(
            "[WARNING] Not enough target customers (minimum 2 required for split). Lowering probability threshold to 0.40..."
        )
    ...
    control_df, variant_df = train_test_split(
        cohort, test_size=0.50, random_state=RANDOM_STATE, shuffle=True
    )
