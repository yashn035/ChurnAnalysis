...
def main():
    ...
    steps = [
        ...
        (
            "src/run_churn_pipeline.py --data data/customer_data.csv",
            "4. Production CLI Runner & Logger",
        ),
    ]
    ...
    expected_files = [
        ...
        "src/generate_tableau_workbook.py",
        "dashboard/Customer_Churn_Dashboard.twb",
        "src/ab_test_cohort.py",
        "data/processed/ab_test_control.csv",
        "data/processed/ab_test_variant.csv",
        "src/run_churn_pipeline.py",
        "data/processed/predictions_output.csv",
        "app/app.py",
        "src/api.py",
        "src/json_logger.py",
        "models/model.pkl",
    ...
    print("=" * 80)
    print(
        f" Workspace Completeness: {found_count} / {len(expected_files)} Files Verified ({found_count/len(expected_files):.0%})"
    )
