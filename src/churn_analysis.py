...
def generate_synthetic_data(
    ...
    if os.path.exists(filepath):
        print(
            f"[INFO] Dataset '{filepath}' already exists. Skipping synthetic generation."
        )
        return pd.read_csv(filepath)

    print(
        f"[INFO] Generating high-fidelity Telco dataset ({num_samples} rows) -> '{filepath}'..."
    )
    ...
    multiple_lines = np.where(
        phone_service == "No",
        "No phone service",
        np.random.choice(["Yes", "No"], size=num_samples, p=[0.45, 0.55]),
    )
    ...
    online_security = np.where(
        internet_service == "No",
        "No internet service",
        np.random.choice(["Yes", "No"], size=num_samples, p=[0.28, 0.72]),
    )
    online_backup = np.where(
        internet_service == "No",
        "No internet service",
        np.random.choice(["Yes", "No"], size=num_samples, p=[0.34, 0.66]),
    )
    ...
    tech_support = np.where(
        internet_service == "No",
        "No internet service",
        np.random.choice(["Yes", "No"], size=num_samples, p=[0.29, 0.71]),
    )
    streaming_tv = np.where(
        internet_service == "No",
        "No internet service",
        np.random.choice(["Yes", "No"], size=num_samples, p=[0.38, 0.62]),
    )
    streaming_movies = np.where(
        internet_service == "No",
        "No internet service",
        np.random.choice(["Yes", "No"], size=num_samples, p=[0.39, 0.61]),
    )
    ...
    base_charge = np.where(
        internet_service == "No", 20.0, np.where(internet_service == "DSL", 50.0, 80.0)
    )
    addons_count = (
    ...
    monthly_charges = np.round(
        base_charge + addons_count * 6.5 + np.random.normal(0, 3, size=num_samples), 2
    )
    monthly_charges = np.clip(monthly_charges, 18.25, 118.75)

    total_charges = np.round(
        monthly_charges * tenure + np.random.normal(0, 25, size=num_samples), 2
    )
    total_charges = np.clip(total_charges, 18.25, 8684.80)

    nan_indices = np.random.choice(
        num_samples, size=int(num_samples * 0.01), replace=False
    )
...
def preprocess_data(
    ...
    if df["TotalCharges"].isnull().sum() > 0:
        ...
        print(
            f"Imputed missing numeric values in 'TotalCharges' with median: {median_val}"
        )
    ...
    df["Churn"] = (
        df["Churn"].astype(str).str.strip().map({"Yes": 1, "No": 0, "1": 1, "0": 0})
    )
...
def evaluate_and_export(
    ...
    try:
        ...
        shap_df = pd.DataFrame(
            {"Feature": selected_feature_names, "Mean_|SHAP|_Value": mean_shap}
    ...
    print(
        "Successfully saved test predictions to 'data/processed/churn_predictions_v2.csv' & 'churn_predictions.csv'."
    )
...
def print_and_save_benchmarks(file_path: str = "benchmarks.json") -> None:
    ...
    print("=" * 80)
    print(
        f"{'Function Name':<30} | {'Execution Time (s)':<20} | {'Peak Memory (MB)':<18}"
    )
    print("-" * 80)
    for b in BENCHMARK_RESULTS:
        print(
            f"{b['function']:<30} | {b['execution_time_seconds']:<20.4f} | {b['peak_memory_mb']:<18.4f}"
        )
    print("=" * 80)
...
def main() -> None:
    ...
    log_pipeline_step(
        logger, "feature_engineering", time.time() - t0, n_samples=len(df)
    )
    ...
    (
        ...
        selector,
        selected_feature_names,
    ) = prepare_features_and_split(df, customer_ids)
    ...
    log_pipeline_step(
        logger, "model_training", time.time() - t0, n_samples=len(X_train_res)
    )
    ...
    log_pipeline_step(
        logger, "prediction_export", time.time() - t0, n_samples=len(y_test)
    )
