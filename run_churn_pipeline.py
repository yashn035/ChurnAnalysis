...
def run_pipeline(data_path, output_path='predictions_output.csv'):
    try:
        ...
        df_model['Churn'] = df_model['Churn'].astype(str).str.strip().map({'Yes': 1, 'No': 0, '1': 1, '0': 0})
        if df_model['Churn'].isnull().any():
            logger.warning("Unmapped values found in 'Churn' target. Filling missing target with 0.")
        ...
        if 'InternetService' in df_model.columns:
            df_model['InternetService'] = df_model['InternetService'].map({'No': 0, 'DSL': 1, 'Fiber optic': 2}).fillna(0).astype(int)
        ...
        if 'tenure' in df_model.columns:
            df_model['tenure_group'] = pd.cut(
                df_model['tenure'], bins=[0, 12, 24, 48, 72], labels=[0, 1, 2, 3], include_lowest=True
            ).fillna(0).astype(int)
            
            if 'TotalCharges' in df_model.columns:
                df_model['avg_monthly_charge'] = np.where(df_model['tenure'] > 0, df_model['TotalCharges'] / df_model['tenure'], 0.0)
                
        if 'OnlineSecurity' in df_model.columns:
            df_model['has_online_security'] = (df_model['OnlineSecurity'] == 'Yes').astype(int)
            
        if 'TechSupport' in df_model.columns:
            df_model['has_tech_support'] = (df_model['TechSupport'] == 'Yes').astype(int)
        ...
        grid_rf = GridSearchCV(RandomForestClassifier(random_state=RANDOM_STATE), param_grid_rf, cv=5, scoring='roc_auc', n_jobs=-1)
